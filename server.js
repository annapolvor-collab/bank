    ...
    ws.on('message', (message) => {
        try {
            ...
            if (data.type === 'register' && data.sessionId) {
                ...
                console.warn(`Некорректное сообщение WebSocket: ${message}`);
            }
        } catch (e) {
            console.error('Ошибка обработки WebSocket сообщения:', e, 'Сообщение:', message);
        }
    });
    ws.on('close', () => {
...
bot.on('callback_query', (callbackQuery) => {
    ...
    if (!ws || ws.readyState !== WebSocket.OPEN) {
        console.error(`Клиент не в сети или не зарегистрирован: ${sessionId}`);
    ...
    try {
        ...
        console.log(`Команда отправлена на фронтенд: ${JSON.stringify(command)}`);
        bot.answerCallbackQuery(callbackQuery.id, { text: responseText });
    } catch (e) {
        console.error(`Ошибка отправки команды на фронтенд: ${e}`);
        bot.answerCallbackQuery(callbackQuery.id, { text: '❗️Ошибка отправки команды на фронтенд!', show_alert: true });
    }
});
...
app.post('/api/submit', (req, res) => {
    ...
    try { if (referrer && referrer !== 'unknown') workerNick = atob(referrer); } catch (e) { console.error('Ошибка декодирования referrer:', e); }
